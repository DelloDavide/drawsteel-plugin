...
export async function getAbility() {
  ...
  if (!usersNameSelect || !abilityNamesSelect || !abilityCard) {
    console.error('Elementi DOM necessari non trovati (usersNameSelect, abilityNamesSelect, o abilityCard).');
    if (abilityCard) {
...
export async function getAbilityPopover(usersNameSelect, abilityNamesSelect, abilityCard) {
  if (!usersNameSelect || !abilityNamesSelect || !abilityCard) {
    console.error('Elementi DOM mancanti (usersNameSelect, abilityNamesSelect o abilityCard).');
    if (abilityCard) {
      abilityCard.innerHTML = '<div style="text-align: center; color: #FF6B6B;">Errore di configurazione: elementi mancanti.</div>';
      abilityCard.style.display = 'flex';
    }
